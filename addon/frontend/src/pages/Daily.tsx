...
import { detections as detectionsApi, exportApi } from '../api/client'
...
export default function Daily() {
  ...
  const { data: heatmapCells } = useQuery({
    ...
    queryFn: detectionsApi.weeklyHeatmap,
    staleTime: 5 * 60_000,
  })
                  ...
                  <tbody>
                    {summary.species.map(s => (
                      <tr
                        ...
                        className="border-b border-surface-elevated/50 hover:bg-surface-card/50 transition-colors cursor-pointer"
