...
import { status as statusApi, events as eventsApi } from '../api/client'
import { deriveChipInfo } from '../hooks/useStatus'
...
function StatusSection({ data }: { data: StatusResponse }) {
  const chip = deriveChipInfo(data)
  ...
  return (
    <div className="card p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-sm font-medium ${chipColors[chip.state]}`}>
          {chip.label}
        </span>
      </div>
      ...
      <div className="flex items-center gap-6 pt-1 text-xs text-slate-500 border-t border-surface-elevated">
        <span>Uptime: <span className="text-slate-400">{formatUptime(data.uptime_seconds)}</span></span>
        <span>Version: <span className="text-slate-400">{data.version}</span></span>
      </div>
    </div>
  )
}
...
function ServiceRow({
  label, ok, detail, okLabel = 'OK', failLabel = 'DOWN',
}: {
  label: string; ok: boolean; detail: string; okLabel?: string; failLabel?: string
}) {
  return (
...
function WamfImportCard() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const resp = await fetch(`${base}/admin/import-wamf`, { method: 'POST', body: form })
